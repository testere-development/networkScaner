...
- **Manual PING scan** — subnet maskaya görə ping ilə cihaz kəşfi
...
## İstifadə
```bash
sudo python tmap.py -aa                 # avtomatik ARP scan (linux)
sudo python tmap.py -ap                 # avtomatik PING scan (linux)
sudo python tmap.py -t 192.168.1.0/24   # manual ARP scan
python tmap.py -sn 24                   # manual PING scan (subnet maska)
python tmap.py -p 192.168.1.1           # full port scan (0-65535)
python tmap.py -p 192.168.1.1:22,80,443 # spesifik portlar
python tmap.py -o hosts.txt             # nəticəni fayla yaz (digər flag-larla birgə)
python tmap.py -h                       # kömək
```
## Tələblər
- Python 3.8+
- `scapy`, `rich`, `requests`

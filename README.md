...
## İstifadə
```bash
sudo python discovery.py -aa                 # avtomatik ARP scan (linux)
sudo python discovery.py -ap                 # avtomatik PING scan (linux)
sudo python discovery.py -t 192.168.1.0/24   # manual ARP scan
python discovery.py -sn 24                   # manual PING scan (subnet maska)
python discovery.py -p 192.168.1.1           # full port scan (0-65535)
python discovery.py -p 192.168.1.1:22,80,443 # spesifik portlar
python discovery.py -o hosts.txt             # nəticəni fayla yaz (digər flag-larla birgə)
